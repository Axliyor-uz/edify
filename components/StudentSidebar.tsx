...
import { 
  ...
  LogOut, Sparkles, Search, Menu, X, Flame, Trophy, ChevronDown, Check 
} from 'lucide-react';
...
// 🟢 Modern Language Selector (Full Width Capable)
const LanguageSelector = ({ lang, setLang, isMobile = false }: { lang: LangType, setLang: (l: LangType) => void, isMobile?: boolean }) => {
...
export default function StudentSidebar({ lang, setLang }: StudentSidebarProps) {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-slate-900 text-slate-300 border-r border-slate-800">
      ...
      {/* 1. BRANDING & CONTROLS AREA (Updated Layout) */}
      <div className="flex flex-col p-5 pb-6 border-b border-slate-800 shrink-0 gap-5">
        ...
        {/* Controls Row (Below Logo) */}
        <div className="flex items-center gap-2">
           {/* Language (Takes available space) */}
           <div className="flex-1 min-w-0">
             <LanguageSelector lang={lang} setLang={setLang} />
           </div>
           
           {/* Notification Bell (Square Button) */}
      ...
      <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              {isActive && (
                <div className="absolute inset-0 bg-gradient-to-r from-slate-800 to-transparent opacity-100" />
              )}
              {isActive && (
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-900/50 shrink-0">
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="bg-slate-800/50 rounded-xl p-3 flex flex-col items-center border border-slate-700/50">
  ...
  return (
    <>
      {/* MOBILE TOP BAR (Visible only when drawer is closed) */}
      <div className="fixed top-0 left-0 right-0 h-16 bg-slate-900 z-40 md:hidden flex items-center justify-between px-4 border-b border-slate-800">
        <div className="flex items-center gap-3">
          <button 
            ...
            className="p-2 text-slate-400 hover:bg-slate-800 rounded-lg active:scale-95 transition-transform"
          >
            <Menu size={24} />
          </button>
        ...
        {/* Mobile Top Actions (Still visible for quick access) */}
        <div className="flex items-center gap-3">
        ...
        {isMobileOpen && (
          <div className="fixed inset-0 z-50 md:hidden">
            ...
            <motion.div 
              ...
              className="absolute inset-y-0 left-0 w-72 bg-slate-900 shadow-2xl border-r border-slate-800"
            >
               <button 
                 onClick={() => setIsMobileOpen(false)}
                 className="absolute top-4 right-[-40px] p-2 bg-slate-800 text-slate-400 hover:text-white rounded-r-lg border-y border-r border-slate-700"
               >
                 <X size={20} />
               </button>
