...
interface Props {
  ...
  student: any;
  assignments: any[];
  classId: string; // 🟢 NEW: Needed to fetch data
}
  ...
  useEffect(() => {
    if (isOpen && student && classId) {
      const fetchStudentGrades = async () => {
        ...
        try {
          // Economy Query: Get ONLY this student's work for THIS class
          const q = query(
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-3xl overflow-hidden flex flex-col h-[90vh] animate-in zoom-in-95 duration-200 shadow-2xl">
          ...
          <div className="flex items-center gap-3">
             ...
             <div>
               <h2 className="text-lg font-black text-slate-800">
                 {viewingAttempt ? 'Review Attempt' : student.displayName}
               </h2>
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50/50 relative">
          ...
          {!viewingAttempt && (
            <div className="p-6 space-y-3">
              <h3 className="text-xs font-bold text-slate-400 uppercase mb-4 flex items-center gap-2">
                <FileText size={14}/> Assignment History
              </h3>
              ...
              {loadingAttempts ? (
                <div className="py-10 flex flex-col items-center justify-center text-slate-400 gap-2">
                  ...
                  <span className="text-xs font-medium">Loading grades...</span>
                </div>
              ) : assignments.length === 0 ? (
                <div className="text-center py-10 text-slate-400 italic">No assignments yet.</div>
              ) : (
                assignments.map((assign) => {
                  // Check assignment restrictions
                  if (Array.isArray(assign.assignedTo) && !assign.assignedTo.includes(student.uid)) return null;

                  // 🟢 MATCH: Check against local 'studentAttempts' state instead of props
                    ...
                    >
                          ...
                          <div className="flex flex-col gap-1 mt-1 text-xs text-slate-500">
                            {assign.dueAt ? (
                              <span className={`flex items-center gap-1 ${isLate ? 'text-red-500 font-bold' : ''}`}>
                                 <Calendar size={10}/> {isLate ? 'Late' : 'Due'}: {formatDate(assign.dueAt)}
                              </span>
                      ...
                      <div className="text-right flex items-center gap-3">
                        {attempt ? (
                          ...
                          <span className="bg-red-50 text-red-600 text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1"><AlertCircle size={14}/> Missing</span>
                        ) : (
                          <span className="bg-slate-100 text-slate-500 text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1"><Clock size={14}/> Pending</span>
                        )}
          ...
          {/* ... (This part remains exactly the same logic as before, just rendering `viewingAttempt`) ... */}
          {viewingAttempt && (
            <div className="p-6">
              {loadingTest || !fullTestData ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
                      ...
                      <div>
                        ...
                        <span className="text-sm text-slate-400 font-bold"> / {viewingAttempt.totalQuestions} Correct</span>
                      </div>
                    ...
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400 mt-1">
                          Attempt #{viewingAttempt.attemptsTaken || 1}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Question Analysis</h3>
                               ...
                               <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                                 <div className={`p-3 rounded-lg border flex flex-col justify-center ${isCorrect ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-red-50 border-red-200 text-red-800'}`}>
                                    <p className="font-bold opacity-60 uppercase mb-1 flex items-center gap-1">
                                      {isCorrect ? <CheckCircle size={12}/> : <XCircle size={12}/>} Student Answer
                                    </p>
                                    <div className="font-mono font-bold text-base flex items-center gap-2">
                                      ...
                                      <span className="text-sm opacity-80 font-medium truncate max-w-[150px]">
                                        <LatexRenderer latex={getContentText(q.options[studentAnswer]) || "Skipped"} />
                                      </span>
                                    </div>
                                 </div>
